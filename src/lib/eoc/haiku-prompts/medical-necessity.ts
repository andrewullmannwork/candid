 ...
 * `type_confidence = 0`, so the fact lands where it landed before P2 (`coverage_rules` — zero regression
 * vs post-D1) but flagged zero-confidence, so the router's confidence-gate (T2) routes it to review.
