...
/** Get the Opus value/excerpt for a (doc, field) — used as reference signal. */
...
function scoreField(args: {
  ...
  const { doc, field, haikuField, ocrText, groundTruth, states } = args;
  ...
  const opusRef = getOpusReference(states, doc, field);
  ...
  // Opus reference (informational; doesn't penalize)
...
export function scoreDoc(args: {
  ...
  for (const field of CANONICAL_PLAN_IDENTITY_FIELDS) {
    ...
    if (extractions.length === 0) {
      ...
      const score = scoreField({ doc, field, haikuField: fakeField, ocrText, groundTruth, states });
      per_field.push(score);
      continue;
    }
    const aggregated = aggregateField(extractions);
    const score = scoreField({ doc, field, haikuField: aggregated, ocrText, groundTruth, states });
    per_field.push(score);
  }
...
export function scoreState(args: {
  state: CalibrationState;
  ocrByDoc: Record<DocSlug, string>;
  groundTruth: GroundTruth;
  allStates: CalibrationState[];
}): StateScore {
  const { state, ocrByDoc, groundTruth, allStates } = args;
  const by_doc: Partial<Record<DocSlug, DocScore>> = {};
  ...
  return { state_id: state.id, state_label: state.label, by_doc };
}
