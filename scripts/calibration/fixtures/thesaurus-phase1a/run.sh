#!/usr/bin/env bash
# Phase 1a T1 fixture runner — spins an EPHEMERAL local Postgres, applies the stub schema +
# the REAL migration 156 + the assertions, reports PASS/FAIL. Non-mutating to PROD (own cluster).
# Requirements: local Postgres tools on PATH (initdb / pg_ctl / psql). Manually runnable:
#   bash scripts/calibration/fixtures/thesaurus-phase1a/run.sh
...
PGDATA="${TMPDIR:-/tmp}/pg_t156_fixture"
PORT="${PG_FIXTURE_PORT:-54401}"
LOG="${TMPDIR:-/tmp}/pg_t156_fixture.log"
...
echo ">>> THESAURUS PHASE-1A T1 FIXTURE: PASS <<<"
