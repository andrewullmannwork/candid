...
3. **Confidence-scaled with per-surface gating** (Pattern 1 #4). Single-source = 0.5, multi-source = 0.9, admin-verified = 1.0. Per-surface display rule: informational surfaces (Plan page, dashboard) show estimated/unverified data with state badges; legal surfaces (Dispute letter, Case File) HIDE non-cite-grade fields entirely per Pattern P-8 cite-grade gate. **Display State v4 + Pattern 1 #16 4-tier badging** (Candid_10k §3.1; Pattern 1 #16 codified Session 80 in Candid_Data_Patterns.md): visible badges = `User Verified` / `Community` / `User Verified + Community` / `Verified` (canonical-level promotion) + `Hidden` (parser failure / boilerplate). Backend `reason` codes preserve cite-grade vs no-cite distinctions for dispute-letter logic (e.g., `from_user_document_cite_grade`, `from_user_document_smart_skip`). Aggregation worst→best: `public_data < community < user_verified < user_verified_community < candid_verified`. Visible badge decoupled from backend confidence on `DecoratedValue<T>` (`badgeLevel` for UI + `backendConfidence` for claim/dispute logic).
