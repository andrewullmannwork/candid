...
3. **Confidence-scaled with per-surface gating** (Pattern 1 #4). Single-source = 0.5, multi-source = 0.9, admin-verified = 1.0. Per-surface display rule: informational surfaces (Plan page, dashboard) show estimated/unverified data with state badges; legal surfaces (Dispute letter, Case File) HIDE non-cite-grade fields entirely per Pattern P-8 cite-grade gate. **Display State v4 (current PROD per S111) → v5 (ACTIVE in candid `main` per S119 B1.3a; PROD promotion deferred)**: v4 visible badges = `User Verified` / `Community` / `User Verified + Community` / `Verified` (canonical-level promotion) + `Hidden`. **v5 ACTIVE** (S119 B1.3a; candid PR #99 shipped to `main`): `consumer-read.ts` DisplayState union extended with `estimate` + 2 new reason codes (`inferred_from_similar_plans`, `cohort_estimate`); `display-state.tsx` adds generalized `pair?: DisplayState` prop on `<DisplayStateBadge>` for dual-pill rendering + `<VerifyPill>` alias export. Split outline-amber into `Public Data` (sourced — CMS / state APCD / NPPES) + `Estimate` (inferred from similar plans / cohort) — both outline amber with differentiated labels. Dual-pill primitive generalized to any non-canonical combination: `[User Verified] [Community]` (CF-40 v4) + new combinations like `[User Verified] [Public Data]`, `[Community] [Estimate]`, `[Public Data] [Estimate]`. Solid green Verified never dual-pills (trumps). Aggregation order v5: `estimate < public_data < community < user_verified < user_verified_community < verified`. Backend `reason` codes preserve cite-grade vs no-cite distinctions for dispute-letter logic (e.g., `from_user_document_cite_grade`, `from_user_document_smart_skip`); v5 added `inferred_from_similar_plans` / `cohort_estimate` reason codes (ACTIVE). Visible badge decoupled from backend confidence on `DecoratedValue<T>` (`badgeLevel` for UI + `backendConfidence` for claim/dispute logic). Full spec at Candid_10k.md §3.1 v5 supplement + plans/s112_phase1_ui_integration.md.
...
8. **User events write user-scoped only; canonical via explicit promotion** (Pattern 1 #14). First-parse uploads, re-parse, manual corrections, value disputes — all write to user rows (`insurance_plans`, `plan_covered_services`, `claim_line_items`). Canonical reference tables (`canonical_plans`, `canonical_plan_services`) populated ONLY via Pattern 2 identity creation OR Pattern 1 #3 promotion event when corroboration threshold met (≥3 distinct EMAIL+PHONE-verified users per Pattern 1 #15 identity gates; migs 074+076 active). Direct user-driven writes to canonical tables are forbidden. Phase 4.0.6 storage-layer authority SHIPPED Session 61 via mig 068 + `apply_promotion_event` Postgres RPC + `canonical_promotion_event_v1` flag global ON; admin attestation lever extended Session 102 via mig 111 (`p_force_event_type='admin_override'`).
...
10. **Case history lives in `claim_case_events` (mig 221).** Mutable rows stay authoritative for CURRENT state; the ledger is authoritative for HISTORY + SEQUENCE. Any NEW mutation site on a claim's case (letter lifecycle, outcomes, escalations, attestations) MUST emit via `src/lib/case/case-events.ts` — fail-soft, flag-gated on `case_timeline_v1`, payloads carry references only (no money, no note text, no PHI). The 18-kind vocabulary is closed in that module; adding a kind is a code change, not a migration.
11. **Operator access to a member's rows goes through `operatorScoped` only** (S330, mig 235). The DFY operator lane never reads or writes a member's data by admin authority: `src/lib/security/operator-scoped.ts` grants access under ONE member's engagement row (`dfy_engagements`), to ONE claim, only while the caller HOLDS the matter (`operator_user_id`), and every executing act asserts the member's own composition events (`ground_selected` + `letter_adopted`) at the ROUTE layer (`src/lib/dfy/operator-action.ts`). Operator acts write `actor: 'operator'` events onto the member's own timeline, references only. `users.is_operator` is the role; never overload `is_admin`.
