...
3. **Confidence-scaled with per-surface gating** (Pattern 1 #4). Single-source = 0.5, multi-source = 0.9, admin-verified = 1.0. Per-surface display rule: informational surfaces (Plan page, dashboard) show estimated/unverified data with state badges; legal surfaces (Dispute letter, Case File) HIDE non-cite-grade fields entirely per Pattern P-8 cite-grade gate. **Display State v4 (current PROD) → v5 (S112 spec target)**: v4 visible badges = `User Verified` / `Community` / `User Verified + Community` / `Verified` (canonical-level promotion) + `Hidden`. **v5 supplement** (S112 D-S112-D lock; Phase 2 implementation target): split outline-amber into `Public Data` (sourced — CMS / state APCD / NPPES) + `Estimate` (inferred from similar plans / cohort) — both outline amber with differentiated labels. Dual-pill primitive generalized to any non-canonical combination: `[User Verified] [Community]` (CF-40 v4) + new combinations like `[User Verified] [Public Data]`, `[Community] [Estimate]`, `[Public Data] [Estimate]`. Solid green Verified never dual-pills (trumps). Aggregation order v5: `estimate < public_data < community < user_verified < user_verified_community < verified`. Backend `reason` codes preserve cite-grade vs no-cite distinctions for dispute-letter logic (e.g., `from_user_document_cite_grade`, `from_user_document_smart_skip`); v5 adds `inferred_from_similar_plans` / `cohort_estimate` reason codes. Visible badge decoupled from backend confidence on `DecoratedValue<T>` (`badgeLevel` for UI + `backendConfidence` for claim/dispute logic). Full spec at Candid_10k.md §3.1 v5 supplement + plans/s112_phase1_ui_integration.md.
