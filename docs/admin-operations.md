...
**The member's side.** Inviting a member opens the engagement, claims it for you, and emails the member a link to `/dfy/<engagement>`, where they read and sign five separate instruments (HIPAA/CMIA authorization · authorized-representative designation · scope of engagement · fee agreement or sponsor-paid disclosure · the health-data consent). Each signature is a consent event plus a PDF filed in the member's own documents. The engagement activates itself once the stack is complete AND the member has composed the appeal in the free tool (fee-free during the pilot; sponsor code; or after the one-time payment when `fee_cents` > 0).
